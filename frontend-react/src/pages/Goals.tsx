  ...
  return (
    <>
      <PageHeader
        ...
        subtitle="Define savings targets, spending reductions, or category caps. Track progress and suggested pace."
      />
                    ...
                    <div className="flex justify-between items-start gap-4">
                      <div>
                        ...
                        <p className="text-text-secondary text-sm mt-1">
                          {formatMoney(g.current_amount ?? 0)} / {formatMoney(g.target_amount ?? 0)} · {g.goal_type}
                        </p>
                        {g.distance && (
                          <p className="text-text-muted text-xs mt-1">Distance: {JSON.stringify(g.distance)}</p>
                        )}
...
function CreateGoalForm({ onCreate }: { onCreate: (p: Parameters<typeof createGoal>[0]) => Promise<void> }) {
  const [goal_type, setGoalType] = useState("savings_target");
    ...
    <div className="card">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <label className="flex flex-col gap-1 md:col-span-2">
          ...
          <input className="input-field" value={description} onChange={(e) => setDescription(e.target.value)} placeholder="e.g. Save for vacation" aria-label="Description" />
        </label>
      </div>
...
function GoalEditForm({
  ...
  const [goal_type, setGoalType] = useState(goal.goal_type || "savings_target");
